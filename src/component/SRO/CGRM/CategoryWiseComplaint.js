// import React from "react";
// import {
//   Box,
// } from "@mui/material";
// import ReactApexChart from "react-apexcharts";

// const CategoryWiseComplaint = ({ data }) => {
//   console.log("CategoryWiseComplaint data:", data);
  
//   // Static categories - yeh hamesha same rahengi
//   const staticCategories = ["Application", "Disbursement", "Dispute with CIR", "Insurance", "Others", "Repayment", "TPP"];

//   // Process the API data structure
//   let series = [];
//   let labels = staticCategories; // Always use static categories for legends

//   if (data && data.chart5 && Array.isArray(data.chart5)) {
//     // API structure: data.chart5 contains array of {label, value, percentage}
//     // Static categories ke according data map karo
//     series = staticCategories.map(staticLabel => {
//       // Find matching data for this static category
//       const matchingItem = data.chart5.find(item => item && item.label === staticLabel);
//       if (matchingItem && matchingItem.percentage) {
//         // Percentage value nikal rahe hain
//         return parseFloat(matchingItem.percentage.replace("%", "")) || 0;
//       }
//       return 0; // If no data found for this category, return 0
//     });
//   } else {
//     // Fallback empty data
//     series = staticCategories.map(() => 0);
//   }

//   console.log("Processed series:", series);
//   console.log("Processed labels:", labels);

//   // Safe member name with fallback
//   const memberName = (data && data.member) ? data.member : 'Member';

//   const pieChartOptions = {
//     chart: {
//       type: "pie",
//       toolbar: {
//         show: true,
//       },
//       animations: {
//         enabled: true,
//         easing: 'easeinout',
//         speed: 800,
//         animateGradually: {
//           enabled: true,
//           delay: 150
//         },
//         dynamicAnimation: {
//           enabled: true,
//           speed: 350
//         },
//         fontFamily: 'sans-serif'
//       }
//     },
//     labels: labels,
//     colors: ["#2B60AD", "#39B1AC", "#69AB44", "#FDBF11", "#F78F6D", "#F05D5F", "#B853A0"],
//     title: {
//       text: "Category wise queries - " + memberName,
//       align: "left",
//       style: { 
//         fontSize: "16px", 
//         fontWeight: "bold", 
//         color: "#263238",
//         fontFamily: 'sans-serif'
//       },
//     },
//     legend: {
//       position: "bottom",
//       horizontalAlign: "center",
//       fontSize: "14px",
//       fontFamily: 'sans-serif',
//       labels: {
//         colors: "#444",
//         useSeriesColors: false
//       }
//     },
//     dataLabels: {
//       enabled: true,
//       formatter: function(val, opts) {
//         const percentage = val.toFixed(2) + "%";
//         const value = data && data.chart5 && data.chart5[opts.seriesIndex] ? data.chart5[opts.seriesIndex].value : "0";
//         return [value, percentage];
//       },
//       style: {
//         fontSize: "11px",
//         colors: ["#fff"],
//         fontWeight: "bold",
//         fontFamily: 'sans-serif'
//       },
//       dropShadow: {
//         enabled: true,
//         top: 1,
//         left: 1,
//         blur: 1,
//         color: '#000',
//         opacity: 0.45
//       }
//     },
//     tooltip: {
//       style: {
//         fontFamily: 'sans-serif'
//       },
//       y: {
//         formatter: function(val, { seriesIndex, w }) {
//           const category = labels[seriesIndex];
//           // Find matching data for tooltip
//           const matchingItem = data && data.chart5 && data.chart5.find(item => item && item.label === category);
//           if (matchingItem && matchingItem.value && matchingItem.percentage) {
//             return matchingItem.value + " (" + matchingItem.percentage + ")";
//           }
//           return val.toFixed(2) + "%";
//         }
//       }
//     },
//     plotOptions: {
//       pie: {
//         dataLabels: {
//           offset: -5,
//           minAngleToShowLabel: 10
//         }
//       }
//     }
//   };

//   return (
//     <Box width="100%">
//       <ReactApexChart
//         options={pieChartOptions}
//         series={series}
//         type="pie"
//         height={400}
//         key={JSON.stringify(series)}
//       />
//     </Box>
//   );
// };

// export default CategoryWiseComplaint;

import React from "react";
import {
  Box,
} from "@mui/material";
import ReactApexChart from "react-apexcharts";

const CategoryWiseComplaint = ({ data }) => {
  console.log("CategoryWiseComplaint data:", data);
  
  // Static categories - yeh hamesha same rahengi
  const staticCategories = ["Application", "Disbursement", "Dispute with CIR", "Insurance", "Others", "Repayment", "TPP"];

  // Process the API data structure
  let series = [];
  let labels = staticCategories; // Always use static categories for legends
  
  // Create a map for easy lookup - IMPORTANT for dataLabels
  const dataMap = {};

  if (data && data.chart5 && Array.isArray(data.chart5)) {
    console.log("✅ Valid chart5 data found");
    
    // First, create a lookup map
    data.chart5.forEach(item => {
      if (item && item.label) {
        dataMap[item.label] = {
          value: item.value,
          percentage: item.percentage
        };
      }
    });
    
    console.log("📋 Data Map created:", dataMap);
    
    // API structure: data.chart5 contains array of {label, value, percentage}
    // Static categories ke according data map karo
    series = staticCategories.map(staticLabel => {
      // Find matching data for this static category
      const matchingItem = dataMap[staticLabel];
      if (matchingItem && matchingItem.percentage) {
        // Percentage value nikal rahe hain
        const percentValue = parseFloat(matchingItem.percentage.replace("%", "")) || 0;
        console.log(`📊 ${staticLabel}: ${matchingItem.value} (${percentValue}%)`);
        return percentValue;
      }
      console.log(`⚠️ No data for ${staticLabel}, using 0`);
      return 0; // If no data found for this category, return 0
    });
  } else {
    console.log("❌ No valid chart5 data");
    // Fallback empty data
    series = staticCategories.map(() => 0);
  }

  console.log("Processed series:", series);
  console.log("Processed labels:", labels);

  // Safe member name with fallback
  const memberName = (data && data.member) ? data.member : 'Member';
  console.log("🏢 Member Name:", memberName);

  const pieChartOptions = {
    chart: {
      type: "pie",
      toolbar: {
        show: true,
      },
      animations: {
        enabled: true,
        easing: 'easeinout',
        speed: 800,
        animateGradually: {
          enabled: true,
          delay: 150
        },
        dynamicAnimation: {
          enabled: true,
          speed: 350
        }
      }
    },
    labels: labels,
    colors: ["#2B60AD", "#39B1AC", "#69AB44", "#FDBF11", "#F78F6D", "#F05D5F", "#B853A0"],
    title: {
      text: "Category wise queries - " + memberName,
      align: "left",
      style: { 
        fontSize: "16px", 
        fontWeight: "bold", 
        color: "#263238",
        fontFamily: 'sans-serif'
      },
    },
    legend: {
      position: "bottom",
      horizontalAlign: "center",
      fontSize: "14px",
      fontFamily: 'sans-serif',
      labels: {
        colors: "#444",
        useSeriesColors: false
      },
      markers: {
        width: 12,
        height: 12,
        radius: 6
      }
    },
    dataLabels: {
      enabled: true,
      formatter: function(val, opts) {
        console.log("🎯 DataLabel formatter called:", { val, seriesIndex: opts.seriesIndex });
        
        // Get the category label for this slice using seriesIndex
        const categoryLabel = staticCategories[opts.seriesIndex];
        console.log("📝 Category Label:", categoryLabel);
        
        // Use dataMap for lookup
        const matchingItem = dataMap[categoryLabel];
        console.log("🔍 Matching Item from dataMap:", matchingItem);
        
        if (matchingItem && matchingItem.value && matchingItem.percentage) {
          // Show value and percentage
          return [matchingItem.value, matchingItem.percentage];
        }
        
        // Fallback
        return ["0", "0%"];
      },
      style: {
        fontSize: "11px",
        colors: ["#fff"],
        fontWeight: "bold",
        fontFamily: 'sans-serif'
      },
      dropShadow: {
        enabled: true,
        top: 1,
        left: 1,
        blur: 1,
        color: '#000',
        opacity: 0.45
      }
    },
    tooltip: {
      style: {
        fontFamily: 'sans-serif'
      },
      y: {
        formatter: function(val, { seriesIndex }) {
          const categoryLabel = staticCategories[seriesIndex];
          
          // Use dataMap for lookup
          const matchingItem = dataMap[categoryLabel];
          
          if (matchingItem && matchingItem.value && matchingItem.percentage) {
            return matchingItem.value + " (" + matchingItem.percentage + ")";
          }
          return val.toFixed(2) + "%";
        }
      }
    },
    plotOptions: {
      pie: {
        dataLabels: {
          offset: -5,
          minAngleToShowLabel: 10
        }
      }
    },
    responsive: [{
      breakpoint: 480,
      options: {
        legend: {
          position: "bottom"
        }
      }
    }]
  };

  return (
    <Box width="100%">
      <ReactApexChart
        options={pieChartOptions}
        series={series}
        type="pie"
        height={400}
        key={JSON.stringify(series)}
      />
    </Box>
  );
};

export default CategoryWiseComplaint;