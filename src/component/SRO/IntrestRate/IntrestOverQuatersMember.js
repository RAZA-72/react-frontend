...
const IntrestOverQuatersMember = ({ intretrateir }) => {
  ...
  const hasDynamicData = intretrateir &&
    intretrateir.graph3_member &&
    intretrateir.graph3_member.length > 0;

  let memberName = defaultData.memberName;
  ...
  if (hasDynamicData) {
    ...
    if (intretrateir.graph3_title && typeof intretrateir.graph3_title === 'string') {
      const parts = intretrateir.graph3_title.split(': ');
      if (parts.length > 1) {
        memberName = parts[1];
      }
    }
    ...
    categories = intretrateir.graph3_member.map(item => item.period);

    series = [
      {
        name: "Min",
        data: intretrateir.graph3_member.map(item => {
  ...
  const lineChartOptions = {
    ...
    title: {
      text: `Interest rates over the quarters: ${memberName}`,
    align: "left",
      style: {
        fontSize: "16px",
        fontWeight: "bold",
...
export default IntrestOverQuatersMember;