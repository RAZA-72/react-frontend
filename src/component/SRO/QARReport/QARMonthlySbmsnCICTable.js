...
function CBMonthlySbmsnCICTable(props) {
  ...
  // 🔒 Always hide first 4 rows (for now)
  const alwaysHidden = getAlwaysHiddenRows();
  filteredRows = filteredRows.filter(
    (row) => !alwaysHidden.includes(row.Parameter)
  );

  // Existing percentage filter
  if (filterType === "percentage") {
    filteredRows = filteredRows.filter(
      (row) =>
        hasPercentage(row["Apr-25"]) ||
