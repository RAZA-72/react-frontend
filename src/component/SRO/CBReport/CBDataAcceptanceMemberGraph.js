...
const CBDataAcceptanceMemberGraph = () => {
  ...
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [memberName, setMemberName] = useState("Member"); // ⭐ NEW
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // ⭐ Set Member Name
        // setMemberName(apiData.short_name || "Member");
        // Set Member Name from API
        setMemberName(
          apiData?.filters?.short_name ? apiData.filters.short_name : "Member"
        );
        ...
        const transformedData = apiData.graph3_data.map(item => {
          ...
          return {
            month: item.Month.replace('31-', '').replace('30-', ''),
            dataSubmissionCount: Math.round(item.Data_Submission_Count_Lk),
            acceptanceRate: Math.round(item.Data_Acceptance_Percent)
          };
    ...
    return (
      <div style={{
        ...
        <h3 style={{
          ...
          Data submission count and acceptance rate - {memberName}
        </h3>
