...
const AdminUsers = () => {
  ...
  const saveAdminUserHandler = async () => {
    ...
    await axios
      .post(`${BaseUrl}/${api}`, state, { headers: authHeaders() })
      .then((response) => {
      ...
      .catch((error) => {
        ...
        if (
          ...
          if (error.response.data.error.mobile_number !== undefined) {
            ErrorToastMessage(
              error.response.data.error.mobile_number[0],
              "failed"
            );
          }
          setState({ ...state, ["isLoader"]: false, ["isDisabled"]: false });
  ...
  useEffect(() => {
    if (filters.search_txt && filters.search_txt.trim() !== "") {
      getAdminUserList();
    }
  }, [filters.search_txt]);
  return (
                  ...
                  <span style={{ float: "right", marginRight: "10px" }}>
                    ...
                    <Dropdown
                      ...
                      menu={[
                        ...
                        <DropdownMenuItem>
                          <Button
                            onClick={downloadPdfMudraDistrictWise}
                            style={{ color: "#000000" }}
                        ...
                        {data.map((user, i) => {
                          return (
                            <tr>
                              ...
                              <td>
                                {user.user_member_info !== null
                                  ? user.user_member_info.MFI_Shortname
                                  : "N/A"}
                              </td>
                              <td>
                                {user.status === 1 ? (
                              ...
                              <td>
                                <Button
                                  onClick={() =>
                                    openEditAdminUserModal(user.id)
                                  }
                                  variant="outlined"
                                  color="primary"
                              ...
                              <td>
                                <Button
                                  onClick={() =>
                                    deleteAdminUserHandler(user.id)
                                  }
                                  variant="outlined"
                                  color="error"
                                  endIcon={<DeleteIcon />}
            ...
            <Grid xs={12} sm={12} md={6}>
              <FormControl variant="standard" sx={{ m: 2, minWidth: 330 }}>
                ...
                >
                  ...
                  {MFIName.map((val, i) => (
                    <MenuItem value={val.id}>{val.MFI_Shortname}</MenuItem>
                  ))}
                </Select>
              </FormControl>
            </Grid>
