...
const TicketSizeChart = ({ data, title = "Ticket Size (%) - All India" }) => {
  ...
  const { series, categories, months, ticketRanges, dynamicMax } = useMemo(() => {
    ...
    // ✅ Filter hidden series aur reverse karo
    ...
    // 🎯 Dynamic max calculation with smart scaling
    let calculatedMax;
    if (maxStackedValue === 0) {
      calculatedMax = 100;
    } else if (maxStackedValue <= 10) {
      // For very small values (0-10%), set max to 15-20
      calculatedMax = Math.ceil(maxStackedValue * 1.0);
    } else if (maxStackedValue <= 30) {
      // For small values (10-30%), add 30-40% padding
      calculatedMax = Math.ceil(maxStackedValue * 1.0);
    } else if (maxStackedValue <= 60) {
      // For medium values (30-60%), add 20-30% padding
      calculatedMax = Math.ceil(maxStackedValue * 1.0);
    } else {
      // For larger values, add 10-15% padding
      calculatedMax = Math.ceil(maxStackedValue * 1.0);
    }
  ...
  // 🔥 Handle chart bar blur effect on hover
  useEffect(() => {
    ...
    bars.forEach((bar, idx) => {
      // DOM bars are in display order (same as series array after reverse)
      // series[0] = Gtr 1.5L (top in display, idx 0 in DOM)
      // series[5] = 0 to 30 (bottom in display, idx 5 in DOM)
  ...
  return (
    <div ref={chartRef}>
      {/* ✅ KEY prop se chart re-render hoga jab series change ho */}
      <ReactApexChart
