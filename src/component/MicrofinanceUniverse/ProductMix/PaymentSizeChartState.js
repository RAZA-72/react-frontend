...
const PaymentSizeChartstate = ({
  ...
  // -------------------------
  // Memoized transform + filter + reverse + DYNAMIC MAX
  // -------------------------
  const { series, categories, months, paymentRanges, dynamicMax } = useMemo(function () {
    var transformed = transformPaymentAPIData(data);

    var filteredSeries = [];
    if (transformed.series && Array.isArray(transformed.series)) {
      filteredSeries = transformed.series
        .filter(function (s) {
    ...
    if (filteredSeries.length > 0 && filteredSeries[0].data) {
      var numDataPoints = filteredSeries[0].data.length;
      
      for (var i = 0; i < numDataPoints; i++) {
        var stackSum = 0;
        for (var j = 0; j < filteredSeries.length; j++) {
          stackSum += (filteredSeries[j].data[i] || 0);
        }
    ...
    return {
      ...
      paymentRanges: transformed.paymentRanges,
  ...
  // -------------------------
  // Apex options (USE dynamicMax here)
  // -------------------------
  var options = {
    chart: {
      type: "bar",
