...
const TicketSizeChartstate = ({
  ...
  // 🔹 Memoized transform + hidden filter + reverse + DYNAMIC MAX
  const { series, categories, months, ticketRanges, dynamicMax } = useMemo(() => {
    const transformed = transformTicketAPIData(data);
    ...
    // 🔥 Calculate dynamic max for y-axis based on visible series
    let maxStackedValue = 0;
    if (filteredSeries.length > 0 && filteredSeries[0].data) {
      const numDataPoints = filteredSeries[0].data.length;
    ...
    // 🎯 Dynamic max calculation with smart scaling - more height/padding
    let calculatedMax;
    if (maxStackedValue === 0) {
      calculatedMax = 100;
    } else if (maxStackedValue <= 10) {
      // For very small values (0-10%), much more space
      calculatedMax = Math.ceil(maxStackedValue * 1.0);
    } else if (maxStackedValue <= 30) {
      // For small values (10-30%), more padding
      calculatedMax = Math.ceil(maxStackedValue * 1.0);
    } else if (maxStackedValue <= 60) {
      // For medium values (30-60%), decent padding
      calculatedMax = Math.ceil(maxStackedValue * 1.0);
    } else {
      // For larger values, moderate padding
      calculatedMax = Math.ceil(maxStackedValue * 1.0);
    }
  ...
  // Custom CSV Download with %
  const downloadCSV = useCallback(() => {
    if (!data || !data.table) return;

    // Create CSV header
    let csv = "category\t";
    ticketRanges.forEach((range, idx) => {
      csv += range.key;
      if (idx < ticketRanges.length - 1) csv += "\t";
    });
    csv += "\n";
    ...
    link.download = "ticket_size_data.csv";
    link.click();
  }, [data, series, categories, ticketRanges]);

  // Override download button
  useEffect(() => {
    const interval = setInterval(() => {
      if (!chartRef.current) return;
      const downloadBtn = chartRef.current.querySelector('.apexcharts-menu-icon.apexcharts-download-icon');
      if (downloadBtn && !downloadBtn.dataset.customized) {
        downloadBtn.dataset.customized = 'true';
  ...
  const options = {
    ...
    yaxis: { show: false, max: dynamicMax },
    grid: { show: false },
    legend: { show: false },
    tooltip: {
      y: { 
