...
const ProductMixMaster = () => {
  ...
  //   toMonth: new Date("2025-09-01"), // ✅ August 2025 (latest)
  //   dateSeries: "All",
  //   maxDate: new Date("2025-09-01"),
  //   region: 0,
  //   states: 0,
  //   isLoader: false,
  //   isDisabled: false,
...
const [graphFilterInitialState] = useState({
  fromMonth: null,
  toMonth: null,
  dateSeries: "All",
  maxDate: null,
  ...
  const [minMonthDate, setMinMonthDate] = useState(null);
  const [maxMonthDate, setMaxMonthDate] = useState(null);
  ...
  var normalizeApiMonth = function (monthStr) {
    if (!monthStr) return null;

    var parts = monthStr.split("-"); // ["Oct","25"]
    ...
    return year + "-" + month; // "2025-10"
  };

  useEffect(function () {
    fetch("https://api.mfinindia.org/api/auth/get-productmix-months")
      .then(function (res) {
        return res.json();
      })
      .then(function (res) {
        if (res.status && Array.isArray(res.months)) {

          // ✅ normalize months
          var normalizedMonths = res.months
            .map(normalizeApiMonth)
            .filter(Boolean)
            .sort(); // important for min/max

          setAvailableMonths(normalizedMonths);

          var min = new Date(normalizedMonths[0] + "-01");
          var max = new Date(
            normalizedMonths[normalizedMonths.length - 1] + "-01"
          );
  ...
  var isMonthAvailable = function (date) {
    return availableMonths.indexOf(formatMonth(date)) !== -1;
  };
    ...
    <>
      <Box sx={{ flexGrow: 1 }}>
        <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} sm={6} md={3}>
                      <DatePicker
                        ...
                        onChange={(date) => {
                          ...
                          if (isMonthAvailable(date)) {
                            handleGraphToDateChange(date);
                          } else {
                            alert("Selected month data not available");
                          }
                        }}
                      />
          ...
          <Typography style={{ textAlign: "left", fontWeight: 500 }}>Note: Due to rounding-off, percentages may not add up to 100%. </Typography>
        </Grid>
      </Box>
    </>
  );
