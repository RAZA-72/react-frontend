...
const SKMList = () => {
  ...
  const fetchData = async (params = {}) => {
    ...
    try {
      ...
      console.log("SKM Filtered Data:", statusFilteredData);


   setRawData(response.data.data);     // full data
setData(statusFilteredData);        // only status-filtered
...
const applyFilters = () => {
  ...
  const exportToExcel = () => {
    ...
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "SKM");

    const randomNum = Math.floor(Math.random() * 9000) + 1000;
    const timestamp = new Date().toISOString().slice(0, 10).replace(/-/g, "");
    const filename = `SKM_${timestamp}_${randomNum}.xlsx`;
  ...
  if (filters.regional_head) {
    ...
    setFilterOptions((prev) => ({
      ...prev,
      mids: [...new Set(filteredMids)], // duplicate remove
    }));
          ...
          <div className="col-12">
            <Box sx={{ display: "flex", alignItems: "center", mb: 2 }}>
              ...
              <Typography variant="h4" component="h1" sx={{ textAlign: 'center', width: '100%' }}>
                SKM Meetings - {status.charAt(0).toUpperCase() + status.slice(1)}
              </Typography>
                  ...
                  <DataTable
                    ...
                    noDataComponent={
                      <Typography sx={{ p: 3, textAlign: "center" }}>
                        No SKM meetings found. Try adjusting your filters.
                      </Typography>
                    }
                    customStyles={{
                      table: {
